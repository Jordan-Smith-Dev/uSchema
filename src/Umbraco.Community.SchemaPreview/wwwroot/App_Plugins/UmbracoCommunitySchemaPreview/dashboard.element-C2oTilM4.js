import { LitElement as f, html as r, css as v, state as d, customElement as x } from "@umbraco-cms/backoffice/external/lit";
import { UmbElementMixin as y } from "@umbraco-cms/backoffice/element-api";
import { UMB_AUTH_CONTEXT as k } from "@umbraco-cms/backoffice/auth";
var w = Object.defineProperty, _ = Object.getOwnPropertyDescriptor, u = (e, t, a, o) => {
  for (var i = o > 1 ? void 0 : o ? _(t, a) : t, n = e.length - 1, s; n >= 0; n--)
    (s = e[n]) && (i = (o ? s(t, a, i) : s(i)) || i);
  return o && i && w(t, a, i), i;
};
function $(e) {
  return e === "Missing @context" ? {
    title: r`Missing <code class="prop-name">@context</code>`,
    ...
    title: r`Missing <code class="prop-name">@type</code>`,
    detail: r`The <code class="prop-name">@type</code> property tells search engines what kind of entity this block represents (e.g. <code class="prop-name">WebPage</code>, <code class="prop-name">Article</code>, <code class="prop-name">Organization</code>).`,
    ...
    "Recommended: itemListElement": {
      title: r`is required for <code class="prop-name">BreadcrumbList</code>`,
      propertyName: "itemListElement",
      detail: r`An ordered array of <code class="prop-name">ListItem</code> entries that make up the breadcrumb trail. Without this the <code class="prop-name">BreadcrumbList</code> block is empty and search engines will ignore it.`,
    ...
    "Recommended: mainEntity": {
      title: r`is required for <code class="prop-name">FAQPage</code>`,
      propertyName: "mainEntity",
      detail: r`An array of <code class="prop-name">Question</code> entities. Without this an <code class="prop-name">FAQPage</code> block has no questions to display and will not generate FAQ rich results in Search.`,
...
const z = "/umbraco/umbracocommunityschemapreview/api/v1", S = r`<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" viewBox="0 0 24 24"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path><path d="M21 3v5h-5"></path></svg>`, g = r`<svg class="btn-icon" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" viewBox="0 0 24 24"><path d="M15 3h6v6M10 14 21 3M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path></svg>`, b = (e) => r`<svg class="btn-icon${e ? " btn-icon--up" : ""}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m4 9 8 8 8-8"></path></svg>`;
let c = class extends y(f) {
  ...
  _onPickerChange(e) {
    ...
    if (typeof a == "string")
      ...
      const s = a[0];
      o = typeof s == "string" ? s : s.unique ?? "";
    }
    const i = o.match(/umb:\/\/[^/]+\/([0-9a-f-]{32,36})/i), n = i ? i[1] : o.trim() || null;
    this._selectedKey = n, this._result = null, this._error = null;
  }
  async _validateSelected() {
    ...
    const e = `${z}/validate/key/${this._selectedKey}`;
    try {
      const t = this._tokenProvider ? await this._tokenProvider() : void 0, a = await fetch(e, {
        headers: {
      ...
      if (!a.ok)
        throw new Error(
          `Validation request failed: ${a.status} ${a.statusText} — URL: ${e}`
        );
      this._result = await a.json();
    } catch (t) {
  ...
  _buildAnnotations(e) {
    ...
    for (const a of e.warnings) {
      const o = a.match(/^Recommended: (\S+)$/);
      o && t.push({ prop: o[1], kind: "warning" });
    }
    return t;
  }
  // ── Render helpers ───────────────────────────────────────────────────────
  _renderSummary() {
    ...
    const e = this._result.summary, t = this._activeFilter, a = (o) => t === null ? "filter-tag" : t === o ? "filter-tag filter-tag--active" : "filter-tag filter-tag--dimmed";
    return r`
  ...
  _renderMessage(e, t, a) {
    const o = $(e), i = o.propertyName ? r`<code class="prop-name">"${o.propertyName}"</code>
                  ${o.title}` : o.title;
    return r`
            <li class="msg-item">
                <div class="msg-header">
                    ...
                    <strong class="msg-title">${i}</strong>
                    ${o.docsUrl ? r`
                              <uui-button
                                  look="primary"
                                  href=${o.docsUrl}
                ...
                ${o.detail ? r`<p class="msg-detail">${o.detail}</p>` : null}
                ${o.example ? r`<pre class="msg-example">${o.example}</pre>` : null}
            </li>
        `;
  }
  _renderJsonBlock(e) {
...
`);
    ...
    const i = a.find((l) => l.trim().startsWith('"')), n = ((m = i == null ? void 0 : i.match(/^(\s+)/)) == null ? void 0 : m[1]) ?? "  ", s = (l) => r`<span class="jv-line">${l + `
`}</span>`, p = ({ prop: l, kind: h }) => r`<span class="jv-line jv-line--ghost jv-line--${h}">${n}<span class="jv-ghost-pill">${h === "error" ? "✗ missing" : "+ add"}</span> &quot;${l}&quot;: ${this._exampleValue(l)}${`
`}</span>`;
    return r`
  ...
  _renderSourceMeta(e) {
    ...
    return (o = this._result) != null && o.hasTemplate && ((i = this._result) != null && i.documentType) ? r`
  ...
  _renderBlock(e) {
    const t = this._blockStatus(e), a = t.charAt(0).toUpperCase() + t.slice(1), i = ["Unknown", "Invalid JSON"].includes(e.type) ? null : `https://schema.org/${e.type}`, n = this._collapsedBlocks.has(e.index);
    return r`
            <uui-box
                class="block-box block-box--${t}${n ? " block-box--collapsed" : ""}"
            >
                ...
                <uui-button
                    ...
                    title=${n ? "Expand this block" : "Collapse this block"}
                >
                    <span class="btn-content">
                        ${n ? "Expand" : "Collapse"}
                        ${b(!n)}
  ...
  _renderResults() {
    if (this._loading)
      return r`<div class="loader"><uui-loader></uui-loader></div>`;
    if (this._error)
      return r`
    ...
    if (!this._result.published)
      return r`
            ...
            `;
    const e = this._filteredBlocks(), t = this._getDuplicateTypes(), { summary: a } = this._result, o = a.valid + a.invalid + a.warnings;
    return r`
            ${this._result.url ? r`
      ...
      (i) => r`<li>
            ...
            <h4 class="section-heading">
                ...
                <span class="section-heading-count">
                    (
                    <strong>${o}</strong>
                    schema block${o !== 1 ? "s" : ""} detected on this
                    page)
                </span>
            </h4>
            ...
            ${this._result.summary.none ? r`
                  ...
                  ` : null}
            ${e.length === 0 && this._activeFilter ? r`
                      <uui-alert>
                          No ${this._activeFilter} blocks on this page.
                      </uui-alert>
                  ` : e.map((i) => this._renderBlock(i))}
        `;
  }
  render() {
    return r`
...
//# sourceMappingURL=dashboard.element-C2oTilM4.js.map
